...
public:
  ...
  assemble()
  {
    ...
    quadrature =
      std::make_unique<dealii::QGauss<SecondRepresentation::support_dimension>>(
        degree + 1);
    ...
    const PrescribedFieldDatum<SecondRepresentation> &datum) const
  {
    ...
    AssertDimension(datum.coefficients().size(), second.dof_handler().n_dofs());
    ...
    for (const auto &particle : particle_coupling.get_particles())
      {
        ...
        first.constraints().distribute_local_to_global(local_matrix,
                                                       first_dof_indices,
                                                       no_column_constraints,
                                                       second_dof_indices,
                                                       coupling_matrix_storage);
