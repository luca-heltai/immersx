...
#  error DEAL_II_WITH_PETSC or DEAL_II_WITH_TRILINOS required
   ...
   * - On boundary ids in `ElasticityProblemParameters::normal_flux_ids`, the
   * code adds constraints via
   *   `VectorTools::compute_nonzero_normal_flux_constraints(...)` using
   *   `ElasticityProblemParameters::Neumann_bc`. (This constrains the net
   * normal flux; see the deal.II function documentation for the exact
   * constraint meaning.)
   *
   ...
   *   `ElasticityProblemParameters::weak_dirichlet_ids`, the code adds a
   * penalty term using \f$h = \f$ `GridTools::minimal_cell_diameter(tria)`: \f[
   ...
   * Point coupling uses Fourier/harmonic modes evaluated at inclusion
   * quadrature points; tensor-product coupling uses the basis from
   * `ReducedCoupling`. In the code, `assemble_coupling()` builds:
   *
   ...
   *
   * Multiplier right-hand side \f$g\f$ (as assembled in `assemble_coupling()`)
   * is a normalized quadrature projection of the inclusion boundary data onto
   * the selected Fourier coefficients. In particular, for each inclusion and
   * each selected mode index \f$j\f$, the code adds terms of the form \f[ g_j
   * \mathrel{+}= \frac{1}{|\Gamma|}\sum_q \psi_j(q)\,d_j(q)\,ds_q, \f] with
   * \f$|\Gamma|=\f$ `section_measure`, basis value \f$\psi_j(q)=\f$
   * `inclusion_fe_values[j]`, and \f$d_j(q)\f$ taken either from
   ...
   * \f]
   * where \f$f_f=B^T\lambda\f$ is present only in the `pressure_coupling ==
   * true` branch (as implemented in `solve()`).
   *
   * Given \f$u^n, v^n=\dot u^n, a^n=\ddot u^n\f$, the code computes:
   * Predictor:
   * \f[
   *   u^{pred} = u^n + \Delta t\,v^n + \frac{\Delta
   * t^2}{2}(1-2\beta_N)a^n,\qquad v^{pred} = v^n + \Delta t(1-\gamma_N)a^n. \f]
   *
   ...
   * \f]
   *
   * @section elasticity_predictor_corrector_matrices Predictor/corrector
   * matrices
   *
   * For matrix-based solvers/preconditioners that work on assembled matrices,
   * the key "effective" (acceleration) matrix used in the Newmark solve is:
  ...
  public:
    ...
    setup_constraints();
    /**
     * Assemble bulk operators (stiffness/mass/damping) and basic right-hand
     * side.
     */
    void
    assemble_elasticity_system();
    ...
    /**
     * Execute the pending refinement/coarsening and transfer on the cells
     * marked by either "refine_and_transfer()" or
     * "refine_and_transfer_around_inclusions()"
     */
    void
    ...
    AffineConstraints<double>
      mean_value_constraints; ///< Mean-value constraints.
    /// @}
