...
class heartMynard
{
 public:
  ...
  void readHeartParameters(string _file);
  void printHeartParameters();
  void end();
...
class leftVentricleModel
{
 public:
  ...
  // methods
  void init(string ifile,string outDir,string testcase);
  void readLeftVentricleParameters(string _file);
  void printLeftVentricleParameters();
  void gete(double t);
  void setAlpha();
  double geteAdim(double t);
  void getPressure();
  void updateState();
  // state
  void saveState();
