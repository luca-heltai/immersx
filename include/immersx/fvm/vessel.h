...
class vessel
{
 public:
  ...
  double initial_node[4];// first value is index of initial node and last 3 are the coordinates of the node
  double final_node[4];// first value is index of final node and last 3 are the coordinates of the node
  ...
  // ####################################################
  // Gravity related variables (General quantities like g or g_intensity are declared in model1d.h)
  // ####################################################
  vector <double> g;  // projection of gravity vector on vessels' axis. It has the size of the computational cells. 
  vector <vector<double>> vessPositSupine; // vector containing the position of the vessels in the 3d space wrt the fixed reference system (supine position). Size: number of computational cells x 3.
  vector <vector<double>> vessVersorPositSupine; // versors containing the position of the vessels in the 3d space wrt the fixed reference system (supine position). Size: number of computational cells x 3.
  vector <vector<double>> vessVersorPositInit; // versors containing the initial position of the vessels in the 3d space as selected in input.
  ...
  double mean_p;
  double mean_a;
  double *psi; // cell auxiliar variable for flow rate spatial gradient (see Montecinos et al., 2014) [m^2/s]
  ...
  double p0;
  double alphaM;
  double Gamma; // Viscoelastic coefficient (constant along vessel) Pa s m (See Alastruey et al., 2011)
  double Gamma_initial; // Viscoelastic coefficient (constant along vessel) Pa s m (See Alastruey et al., 2011)
  ...
  int vasTerInd; // 0 to nTerminals index of vascular territory to which the vessel belongs
  ...
  // lymphatics
    ...
    void getLymphaticsAuxVariables(double *q);
    void getLymphaticsTimeDerivative(double *q, double *dqdt);
  ...
  // methods
  virtual double geta0(const int& iCell, const int& iDofs);
  virtual double getaRef(const int& iCell, const int& iDofs);
  virtual double geth0(const int& iCell, const int& iDofs);
  virtual double getee(const int& iCell, const int& iDofs);
  virtual double getec(const int& iCell, const int& iDofs);
  virtual double getep0(const int& iCell, const int& iDofs);
  virtual double getepr(const int& iCell, const int& iDofs);
  virtual double getpe(const int& iCell, const int& iDofs);

  virtual double geta0av(const int& iCell);
  virtual double getaRefav(const int& iCell);
  virtual double geth0av(const int& iCell);
  virtual double geteeav(const int& iCell);
  virtual double getecav(const int& iCell);
  virtual double getep0av(const int& iCell);
  virtual double geteprav(const int& iCell);
  virtual double getpeav(const int& iCell);
  
  virtual void seta0const(const int& iCell, const double& val);
  virtual void setaRefconst(const int& iCell, const double& val);
  virtual void seth0const(const int& iCell, const double& val);
  virtual void seteeconst(const int& iCell, const double& val);
  virtual void setecconst(const int& iCell, const double& val);
  virtual void setep0const(const int& iCell, const double& val);
  virtual void seteprconst(const int& iCell, const double& val);
  virtual void setpeconst(const int& iCell, const double& val);

  virtual void multa0const(const int& iCell, const double& val);
  virtual void multaRefconst(const int& iCell, const double& val);
  virtual void multh0const(const int& iCell, const double& val);
  virtual void multeeconst(const int& iCell, const double& val);
  virtual void multecconst(const int& iCell, const double& val);
  virtual void multep0const(const int& iCell, const double& val);
  virtual void multeprconst(const int& iCell, const double& val);
  virtual void multpeconst(const int& iCell, const double& val);
