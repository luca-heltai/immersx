...
#include <immersx/fvm/GetPot.h>
#include <immersx/fvm/cblas.h>
#include <immersx/fvm/linalg.h>
 ...
 * @note equations are analogous to those presented for cerebral autoregulation in
    Toro, E. F., Celant, M., Zhang, Q., Contarino, C., Agarwal, N., Linninger,
 A., & Müller, L. O. (2022). Cerebrospinal fluid dynamics coupled to the global
 circulation in holistic setting: mathematical models, numerical methods and
 applications. International Journal for Numerical Methods in Biomedical
 Engineering, 38(1), e3532.
...
public:
  ...
  // algebraic variables, each component correspond to one terminal vessel in
  ...
  void
  getAutoregulationParameters(string _file);
  void
  printAutoregulationParameters();
  ...
  void
  getState(vector<vector<double>> &state);
  void
  setState(vector<vector<double>> &state);
