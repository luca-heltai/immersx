 ...
 */
template <typename FieldVectorType, typename GlobalVectorType = FieldVectorType>
...
public:
  ...
  using LinearSolveFunction = std::function<
    void(const Action &, const GlobalVectorType &, GlobalVectorType &, double)>;
  using HistoryQuery =
    ...
    if constexpr (std::is_same_v<FieldVectorType, GlobalVectorType>)
      {
        ...
        current_action_ = Action([model,
                                  ...
                                  derivative_fields =
                                    std::move(derivative_fields)](
                                   GlobalVectorType       &destination,
