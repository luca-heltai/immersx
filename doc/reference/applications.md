...
| `fiber_reinforced_elastodynamics` | `apps/app_fiber_reinforced_elastodynamics.cc`; matrix `ElastodynamicsSolver<dim,dim>` and embedded line `ElastodynamicsSolver<1,dim>` with a multiplier constraint | `2/2` and `3/3` matrix meshes with a 1D-in-`dim` fiber | MPI; `tutorials/fiber_reinforced_elastodynamics/parameters.prm.in` |
