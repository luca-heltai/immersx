...
## Reduced elasticity migration

The preferred immersed-coupling infrastructure is `ReducedCoupling`, backed by
`TensorProductSpace`, `ReferenceCrossSection`, `ParticleCoupling`, and `VTKUtils`.
`ReducedPoisson` is the reference implementation. New elasticity geometries should
use VTK reduced grids and the vector-valued reduced coupling parameters.

Elasticity retains the legacy `Inclusions` implementation as an opt-in fallback.
Set `Use reduced coupling = true` under `Immersed Problem` to use the VTK path.
Reduced coupling currently supports one setup/solve cycle; adaptive refinement is
explicitly rejected until reduced-particle transfer is implemented.
...
During development, run the focused migration tests with:

```bash
./build-debug/gtests/gtests_debug --gtest_filter='*ElasticityReducedCoupling*'
```
...
input through the reduced-coupling infrastructure.
