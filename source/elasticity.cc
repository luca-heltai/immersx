...
#include <immersx/algebra/augmented_lagrangian.h>
#include <immersx/algebra/augmented_lagrangian_preconditioner.h>
#include <immersx/algebra/solver_controls.h>
...
namespace ImmersX
{
      ...
      if (cell->is_locally_owned())
        {
                        ...
                        for (unsigned int i = 0; i < dofs_per_cell; ++i)
                          {
                            const auto comp_i =
                              fe->system_to_component_index(i).first;
                            const auto un = neumann_bc.value(
                              fe_face_values.quadrature_point(q), comp_i);
                            cell_rhs(i) += un *
          ...
          // We don't want penalty terms in the mass and damping matrices, but
          // we do want them in the stiffness and newmark matrices
            ...
            if (cell->face(f)->at_boundary())
              {
                ...
                // Neumann Boundary conditions
                         ...
                         par.neumann_ids.end())
                  {
                    const auto &neumann_bc =
                      par.get_neumann_bc(cell->face(f)->boundary_id());
                    const auto neumann_scale =
                      neumann_bc.scale(evaluation_time);
                    fe_face_values.reinit(cell, f);
                    for (unsigned int q = 0;
                         q < fe_face_values.n_quadrature_points;
                         ++q)
                ...
                for (unsigned int j = 0; j < inclusions.n_dofs_per_inclusion();
                     ++j)
                  {
                    ...
                    if (inclusions.inclusions_data[inclusion_id].size() > 0)
                      {
                        if (inclusions.inclusions_data[inclusion_id].size() +
                              1 >
        ...
        if (par.pressure_coupling == false)
          {
            ...
            { // auto interp_g = g;
              ...
              SolverMinRes<LA::MPI::Vector> cg_schur(
                par.schur_complement_solver_control);
              ...
              pcout << "   Solved for lambda in "
                    << par.schur_complement_solver_control.last_step()
                    << " iterations." << std::endl;
  ...
  ElasticityProblem<dim, spacedim>::solve_quasistatic()
  {
    ...
    if (n_multiplier_dofs() == 0)
    ...
    else
      {
        ...
        const auto amgM = linear_operator(M, prec_M);
        SolverCG<LA::MPI::Vector> cg_M(par.reduced_mass_solver_control);
        const auto                invM = inverse_operator(M, cg_M, amgM);
  ...
  ElasticityProblem<dim, spacedim>::solve_newmark()
  {
    ...
    AssertThrow(par.pressure_coupling == true ||
                  uses_tensor_product_coupling() || n_multiplier_dofs() == 0,
                ExcNotImplemented(
                  "Dynamic solve with pressure_coupling == false "
        ...
        if (par.time_mode == TimeMode::Static && cycle == 0 && time_step == 0)
          {
            ...
            output_immersed_particles(par.output_directory + "/" +
                                      particles_filename);
        ...
        else if (par.time_mode != TimeMode::Static)
          {
            ...
            particles_filename << par.output_name << "_particles_" << cycle
            ...
            const std::string pvd_particles_name =
              par.output_directory + "/" + par.output_name +
              "_particles_cycle_" + std::to_string(cycle) + ".pvd";
    ...
    for (cycle = 0; cycle < par.n_refinement_cycles; ++cycle)
      {
        ...
        // Build the C matrix AMG only for the initial acceleration solve
        {
          Teuchos::ParameterList parameter_list_C;
          parameter_list_C.set("smoother: type", "Chebyshev");
          parameter_list_C.set("smoother: sweeps", 2);
          parameter_list_C.set("smoother: pre or post", "both");
          parameter_list_C.set("coarse: type", "Amesos-KLU");
          parameter_list_C.set("coarse: max size", 2000);
          parameter_list_C.set("aggregation: threshold", 0.02);
        ...
        par.exact_solution.set_time(current_time);
        par.convergence_table.error_from_exact(
          dh, locally_relevant_solution.block(0), par.exact_solution);
