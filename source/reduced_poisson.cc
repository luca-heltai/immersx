...
#  endif
        ...
        else if (par.solver_name == "AL")
          {
            ...
            if constexpr (reduced_dim > 0)
              DoFTools::make_sparsity_pattern(
                reduced_coupling.get_dof_handler(), dsp_reduced_mass);
            else
            ...
            pcout << "Reduced mass matrix size: " << reduced_mass_matrix.m()
                  ...
                  << ", norm: " << reduced_mass_matrix.linfty_norm()
                  << std::endl;


            const auto M =
              linear_operator<LA::MPI::Vector>(reduced_mass_matrix);
            ...
            TrilinosWrappers::MPI::Vector
              inverse_squares_reduced; // diag(M)^{-2}
            ...
            if (par.estimate_condition_number)
              {
                ...
                pcout << "- - - - - - - - - - - - - - - - - - - - - - - -"
                      << std::endl;
                pcout << "Estimate condition number of BBt using CG"
                      << std::endl;
                SolverControl solver_control(100000, 1e-12);
                SolverCG<TrilinosWrappers::MPI::Vector> solver_cg(
                  solver_control);
    ...
    print_parameters() const
  {
    ...
    if (Utilities::MPI::this_mpi_process(mpi_communicator) == 0)
      {
        par.prm.print_parameters(par.output_directory + "/" +
                                   "used_parameters_" + std::to_string(dim) +
    ...
    for (cycle = 0; cycle < par.n_refinement_cycles; ++cycle)
      {
        ...
        // coupling_operator =
        // std::make_unique<MatrixFreeCouplingOperator<spacedim,
        // double>>(
