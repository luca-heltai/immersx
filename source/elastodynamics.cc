  ...
  , convergence_table(std::vector<std::string>(spacedim, "d"))
{
  ...
  body_force.declare_parameters_call_back.connect(declare_zero_vector_function);
...
ElastodynamicsSolver<dim, spacedim>::setup_system()
{
  ...
  locally_relevant_displacement.reinit(owned_dofs,
                                       relevant_dofs,
                                       mpi_communicator);
  locally_relevant_velocity.reinit(owned_dofs, relevant_dofs, mpi_communicator);
...
ElastodynamicsSolver<dim, spacedim>::set_initial_conditions()
{
  ...
  VectorTools::interpolate(dh, par.initial_displacement, displacement_storage);
...
ElastodynamicsSolver<dim, spacedim>::advance_one_timestep(const double dt)
{
  ...
  assemble_backward_euler_system(previous_displacement, previous_velocity, dt);
