                   ...
                   "Solver/Control")
{
  ...
  add_parameter("Analytical solution expression",
...
template <int dim, int spacedim>
bool
NavierStokesSolver<dim, spacedim>::uses_fully_distributed_triangulation() const
{
...
NavierStokesSolver<dim, spacedim>::setup_system()
{
  ...
  AssertThrow(dofs_per_block.size() == 2 && dofs_per_block[0] > 0 &&
                dofs_per_block[1] > 0,
              ExcMessage("The velocity/pressure DoF blocks must be nonempty."));
  ...
  owned_dofs_by_block[1] = owned_dofs.get_view(dofs_per_block[0], dh.n_dofs());
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
          {
            ...
            const Tensor<1, spacedim> convective = par.include_convective_term ?
                                                     old_grad_u[q] * old_u[q] :
                                                     Tensor<1, spacedim>();
...
NavierStokesSolver<dim, spacedim>::output_results() const
{
  ...
  if (Utilities::MPI::this_mpi_process(mpi_communicator) == 0)
    {
      std::ofstream pvd(par.output_directory + "/" + par.output_name + ".pvd");
...
{
  ensure_output_directory(par.output_directory);
  pcout << "Running NavierStokesSolver<" << Utilities::dim_string(dim, spacedim)
        << ">." << std::endl;
    ...
    if (cell->is_locally_owned())
      {
        ...
        for (unsigned int q = 0; q < quadrature->size(); ++q)
          local_integral += divergences[q] * divergences[q] * fe_values.JxW(q);
      }
