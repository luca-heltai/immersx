...
namespace ImmersX
{
    ...
    if (inclusions.n_dofs() > 0)
      {
        ...
        mass_matrix.reinit(owned_dofs[1],
                           owned_dofs[1],
                           idsp,
                           mpi_communicator);
      }
  ...
  PoissonProblem<dim, spacedim>::assemble_coupling()
  {
    ...
    [[maybe_unused]] FullMatrix<double> local_bulk_matrix(
      fe->n_dofs_per_cell(), fe->n_dofs_per_cell());
...
#endif
        const auto C =
          TrilinosWrappers::linear_operator<VectorType>(inclusion_matrix);
